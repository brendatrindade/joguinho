...
int main() {
    ...
    // DESENHA OU APAGA LABIRINTO
    for (int i = 0; i < 1500; i++) {
            imprimeLabirintoVGA();
            //apagaLabirinto();
    }
    
    pthread_t thread_a, thread_m;
    
    if (pthread_create(&thread_a, NULL, colisao_labirinto, NULL) != 0) {
        perror("Erro ao criar thread");
        return 1;
    }
    if (pthread_create(&thread_m, NULL, move_sprite, NULL) != 0) {
        perror("Erro ao criar thread");
        return 1;
    }
